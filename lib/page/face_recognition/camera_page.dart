...
class _FaceScanScreenState extends State<FaceScanScreen> {
  ...
  Future<void> _predictFacesFromImage({required CameraImage image}) async {
    ...
    if (facesDetected.isNotEmpty) {
      ...
      print('The data of user:');
      print(user?.toJson());
      if (widget.user == null) {
        // register case
        Navigator.pop(context);
        customBottomSnackbar(context, 'User registered successfully');
        print("User registered successfully");
      } else {
        // login case
        if (user == null) {
          Navigator.pop(context);
          customBottomSnackbar(context, 'Unknown User');
          print("Unknown User");
        } else {
      ...
      customBottomSnackbar(context, "No face detected");
    }
  ...
  @override
  void initState() {
    print('the widget.user is = ${widget.user?.toJson()}');
                      ...
                      Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            GestureDetector(
                              onTap: () async{
                                print("changing camera lens direction");
                                _cameraController =  CameraController(cameras![0], ResolutionPreset.high);
                                await _cameraController.initialize();
                                _cameraController.setFlashMode(FlashMode.off);
                                setState(() {});
                              },
                                child: Container(padding: EdgeInsets.symmetric(horizontal: 10),child: Icon(Icons.camera,color: Colors.black,size: 35,),))
                          ],
                        ),
                      ),
                      IconButton(
